<div align="center">

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=500&size=22&pause=1000&color=6E56CF&center=true&vCenter=true&width=520&lines=Hey%2C+I'm+Naveenkumar+%F0%9F%91%8B;Building+AI+safety+tooling;RAG+pipelines+%26+local+LLMs;Backend+engineer+%40+Pando)](https://github.com/naveenkumar-chandrasekar)

[![Profile Views](https://komarev.com/ghpvc/?username=naveenkumar-chandrasekar&color=6E56CF&style=flat-square&label=profile+views)](https://github.com/naveenkumar-chandrasekar)

</div>

---

```ts
const naveenkumar = {
  role:    "SDE @ Pando · Ex-BYJU'S · Chennai 🇮🇳",
  focus:   ["LLM guardrails", "RAG systems", "local-first AI"],
  stack:   {
    backend:  ["Node.js", "Fastify", "Express", "TypeScript", "Python"],
    frontend: ["Vue 3", "React", "Vite", "Pinia"],
    ai:       ["Ollama", "Claude", "OpenAI", "pgvector", "RAG"],
    data:     ["PostgreSQL", "MongoDB", "Drizzle ORM"],
    infra:    ["Docker", "AWS", "GitHub Actions", "Datadog"],
  },
  npm:     "llm-guardrails",
  writing: "naveenkumar-c.medium.com",
};
```

---

## Projects

| | What it does |
|---|---|
| [**Askfolio-RAG**](https://github.com/naveenkumar-chandrasekar/Askfolio-RAG) | Upload docs, ask questions, get streamed answers with citations |
| [**FinSight**](https://github.com/naveenkumar-chandrasekar/FinSight) | AI finance tracker — import bank statements, auto-categorize, chat via Ollama |
| [**llm-guardrails-middleware**](https://github.com/naveenkumar-chandrasekar/LLM-guardrails-middleware) | Express middleware — PII redaction, prompt injection detection, toxicity filtering |
| [**LLM-guardrails-Demo**](https://github.com/naveenkumar-chandrasekar/LLM-guardrails-Demo-Chat-app) | Chat app showcasing safety policies in action |

---

## npm

[![npm](https://img.shields.io/npm/v/llm-guardrails?color=6E56CF&label=llm-guardrails&logo=npm&logoColor=white&style=flat-square)](https://www.npmjs.com/package/llm-guardrails)
[![npm downloads](https://img.shields.io/npm/dm/llm-guardrails?color=6E56CF&style=flat-square&logo=npm&logoColor=white)](https://www.npmjs.com/package/llm-guardrails)

LLM safety middleware for Node — PII, injection, toxicity, schema validation, and policy engine.

```bash
npm install llm-guardrails
```

---

## Stats

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=naveenkumar-chandrasekar&show_icons=true&hide_border=true&count_private=true&bg_color=0d1117&title_color=6E56CF&icon_color=6E56CF&text_color=c9d1d9)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=naveenkumar-chandrasekar&layout=compact&hide_border=true&bg_color=0d1117&title_color=6E56CF&text_color=c9d1d9)

![Streak](https://github-readme-streak-stats.herokuapp.com/?user=naveenkumar-chandrasekar&hide_border=true&background=0d1117&ring=6E56CF&fire=6E56CF&currStreakLabel=6E56CF&sideLabels=c9d1d9&dates=c9d1d9&sideNums=c9d1d9&currStreakNum=c9d1d9)

</div>

---

## Writing

Backend systems, databases, and DevOps on Medium → [naveenkumar-c.medium.com](https://naveenkumar-c.medium.com)

Topics: Redis · Docker · PostgreSQL · OpenID / OAuth / SAML

---

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/naveenkumar-c-s/)
[![Medium](https://img.shields.io/badge/Medium-000000?style=for-the-badge&logo=medium&logoColor=white)](https://naveenkumar-c.medium.com)
[![npm](https://img.shields.io/badge/npm-CB3837?style=for-the-badge&logo=npm&logoColor=white)](https://www.npmjs.com/~naveenkumar-c-49)

</div>
